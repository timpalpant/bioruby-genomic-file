...
describe UCSC::TrackHeader do  
  context "when instantiated in code" do
    before do
      @test = UCSC::TrackHeader.new(:type => 'bedGraph', :name => 'test header', :description => 'test description', :view_limits => '0:1')
    end
  ...
  context "when parsed from a track line" do
    before do
      @test = UCSC::TrackHeader.parse("track type=wiggle_0 name=\"My Test Wig\" description='Test Wig for Running Specs' viewLimits=0:1 autoScale=off visibility='full'")
    end
