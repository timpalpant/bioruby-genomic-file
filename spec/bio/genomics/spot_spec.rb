...
describe Genomics::Spot do  
  context "with an ID" do
    before do
      @test = Genomics::Spot.new("chrI", 1, 100, "MySpot")
    end
  ...
  context "with an ID and a value" do
    before do
      @test = Genomics::Spot.new("chrI", 1, 100, "MySpot", 10.0)
    end
  ...
  context "with a value but no ID" do
    before do
      @test = Genomics::Spot.new("chrI", 55, 50, nil, 10.0)
    end
