  ...
  module Utils
    module UCSC
      ...
      def self.bigwig_summary(f, chr, start, stop, num_values, type = 'mean')
        ...
        raise ToolsError, "BigWig does not contain data for the interval #{chr}:#{start}-#{stop}" if output.start_with?('no data in region')
      ...
      def self.twobit_to_fa(twobit_file, chr, start = nil, stop = nil)
        ...
        if (start and stop) and seq.length != (stop-start+1)
          raise ToolsError, "twoBitToFa did not return the expected sequence length!" 
        end
      ...
      class TrackHeader
        ...
        def set(key, value)
          ...
          case key
            ...
            when 'smoothingWindow'
              ...
              raise TrackHeaderError, "Unknown UCSC track header key #{key}"
          end
        end
        ...
        def self.parse(line)
          unless line.chomp.start_with?('track')
            raise TrackHeaderError, "Not a valid UCSC Genome Browser track line"
          end
      ...
      class TrackHeaderError < StandardError
      end
    end
  end
end
